...
def test():
    # Board
    print()
    board = Board((Player('A', PIECES_COLORS[0], 'N'), Player('B', PIECES_COLORS[1], 'S')))
    for n in reversed(range(8)):
        for square in board.get_row(n):
            print (square.name, end = ' ')
        print('\t(row' + str(n) + ')')
    print()
    for n in reversed(range(8)):
        for square in board.get_row(n):
            print (square.color[0:5], end = ' ')
        print()
    print()
    for n in reversed(range(8)):
        for square in board.get_row(n):
            if square.piece == None:
                ...
                print (square.piece.name.center(5), end = ' ')
        print()
    # Square
    print()
